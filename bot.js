...
const onMessage = async (message) => {
  ...
  if (room) {
    ...
    if (config.groups.includes(topic) && followKeyWords.some((t) => text.includes(t))) {
      if (!text.startsWith('<msg>') && text.length <= 100 && !text.includes('- - - - - - - - - - -') && memberCallCds.findIndex((item) => item.name === text) == -1) {
        await room.say(text)
        memberCallCds.push({ name: text, timer: setTimeout(() => (memberCallCds = memberCallCds.filter((m) => m.name != text)), 6000) })
      }
    }
  }
}
